...
def create_simple_templates():
        ...
        <body>
            ...
            <div class="container mt-4">
                ...
                <div class="card mt-4">
                    ...
                    <div class="card-body">
                        <p>جار تحميل سجلات المراقبة...</p>
                        <div id="monitorLogs"></div>
                    </div>
                </div>
            </div>
            ...
            <script>
                ...
                window.addEventListener('load', async () => {
                    ...
                    if (result.success && result.data.length > 0) {
                        ...
                        let html = '<div class="table-responsive"><table class="table table-sm"><thead><tr><th>الوقت</th><th>النوع</th><th>الحالة</th><th>التفاصيل</th></tr></thead><tbody>';
                        ...
                        result.data.forEach(check => {
                            ...
                            html += `
                                <tr>
                                    <td>${check.timestamp}</td>
                                    <td>${check.check_name}</td>
                                    <td><span class="badge bg-${statusBadge}">${check.status}</span></td>
                                    <td>${check.details}</td>
                                </tr>
                            `;
                        });
        ...
        <body>
                    ...
                    <div class="col-md-6">
                        <div class="card">
                            ...
                            <div class="card-body" id="quickStats">
                                <p class="text-muted">جار تحميل الإحصائيات...</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <script>
                ...
                async function loadQuickStats() {
                    ...
                    // استخراج الإحصائيات من الصفحة (طريقة مبسطة)
                    const parser = new DOMParser();
                    const doc = parser.parseFromString(text, 'text/html');
                    const statsDiv = doc.querySelector('.stats-card');
                    
                    if (statsDiv) {
                        document.getElementById('quickStats').innerHTML = statsDiv.innerHTML;
                    }
                }
    ...
    for template_name, content in simple_templates.items():
